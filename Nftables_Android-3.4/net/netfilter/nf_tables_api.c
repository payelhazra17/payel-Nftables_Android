...
{
	struct sk_buff *skb;
	u32 portid = oskb ? NETLINK_CB(oskb).pid : 0;
			      ...
			      const struct nlattr * const nla[])
{
	...
	err = nf_tables_fill_table_info(skb2, NETLINK_CB(skb).pid,
		...
		list_for_each_entry(table, &afi->tables, list) {
			list_for_each_entry(chain, &table->chains, list) {
				...
				if (idx > s_idx)
					memset(&cb->args[1], 0,
					       sizeof(cb->args) - sizeof(cb->args[0]));
				if (nf_tables_fill_chain_info(skb, NETLINK_CB(cb->skb).pid,
							      cb->nlh->nlmsg_seq,
							      NFT_MSG_NEWCHAIN,
							      NLM_F_MULTI,
			      ...
			      const struct nlattr * const nla[])
{
	...
	err = nf_tables_fill_chain_info(skb2, NETLINK_CB(skb).pid,
...
{
	struct sk_buff *skb;
	u32 portid = NETLINK_CB(oskb).pid;
	struct net *net = oskb ? sock_net(oskb->sk) : &init_net;
			...
			list_for_each_entry(chain, &table->chains, list) {
				list_for_each_entry(rule, &chain->rules, list) {
					...
					if (idx > s_idx)
						memset(&cb->args[1], 0,
						       sizeof(cb->args) - sizeof(cb->args[0]));
					if (nf_tables_fill_rule_info(skb, NETLINK_CB(cb->skb).pid,
								      cb->nlh->nlmsg_seq,
								      NFT_MSG_NEWRULE,
			     ...
			     const struct nlattr * const nla[])
{
	...
	err = nf_tables_fill_rule_info(skb2, NETLINK_CB(skb).pid,
			      ...
			      const struct nft_set *set, u16 event, u16 flags)
{
	struct nfgenmsg *nfmsg;
	struct nlmsghdr *nlh;
	u32 portid = NETLINK_CB(ctx->skb).pid;
